...
class FotoTab(QWidget):
    ...
    a file dialog with validation and conflict resolution.
    ...
    def _connect_signals(self) -> None:
        ...
        self._table.cellDoubleClicked.connect(self._on_double_click_photo)

    def _setup_ui(self) -> None:
        """Create the UI layout with button row, table, and empty state."""
        ...
        btn_layout.addWidget(self._edit_button)
    ...
    def _on_photo_selected(self) -> None:
        ...
        Updates the selected media item, enables/disables the delete button,
    ...
    def _on_edit_photo(self) -> None:
        """Open EditPhotoDialog for the selected photo."""
        if self._selected_media_item is None:
            return
        ...
        updated = dialog.get_updated_media_item()
        if updated is not None:
            self.refresh()

    def _on_double_click_photo(self, row: int, column: int) -> None:
        """Open EditPhotoDialog when a photo row is double-clicked."""
        typ_item = self._table.item(row, 0)
        if typ_item is None:
            return
        media_id = typ_item.data(Qt.ItemDataRole.UserRole)
    ...
    def refresh(self) -> None:
        ...
        # If no re-selection, hide preview and reset state
        self._close_preview_window()
        self._selected_media_item = None
        self._edit_button.setEnabled(False)
        self._delete_button.setEnabled(False)
